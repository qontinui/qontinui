...
class PixelStabilityMatrixAnalyzer:
    ...
    def analyze_screenshots(
        ...
        for pattern_key, pixel_coords in pattern_groups.items():
            ...
            components = self._find_connected_components(pixel_coords, screenshots[0].shape[:2])
    ...
    def _build_stability_matrix(
        ...
        for y in range(y_start, y_end):
            for x in range(x_start, x_end):
                ...
                if len(presence_set) >= self.config.min_screenshots_present:
    ...
    def _pixels_match(self, pixel1: np.ndarray, pixel2: np.ndarray) -> bool:
    ...
    def _group_by_pattern(
        self, stability_matrix: dict[tuple[int, int], tuple[set[int], np.ndarray]]
    ...
    def _create_state_image(
        ...
        mask_density = np.sum(mask) / (width * height)
        ...
        # Store mask information (would need to extend StateImage model)
        # For now, we can add it to tags to indicate it has a mask
        if mask_density < 0.95:  # Not a full rectangle
            state_image.tags.append(f"mask_density_{mask_density:.2f}")
            state_image.tags.append("has_mask")
    ...
    def _calculate_pixel_percentages(
        self, region: np.ndarray, mask: np.ndarray | None = None
        ...
        light_pct = (light_count / total_pixels) * 100
