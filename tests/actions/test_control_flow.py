...
from qontinui.actions.control_flow.condition_evaluator import compare_values
...
def mock_action_executor():
    ...
    async def executor(action_id: str, variables: dict[str, Any]) -> dict[str, Any]:
...
async def test_for_loop_basic(mock_action_executor):
    ...
    result = await executor.execute_loop(action)
...
async def test_for_loop_multiple_actions(mock_action_executor):
...
async def test_for_loop_no_iterations():
...
async def test_for_loop_max_iterations_limit(mock_action_executor):
...
async def test_for_loop_break_on_error(failing_action_executor):
...
async def test_for_loop_continue_on_error(failing_action_executor):
...
async def test_for_loop_break(break_action_executor):
...
async def test_for_loop_continue(continue_action_executor):
...
async def test_while_loop_variable_condition(mock_action_executor):
...
async def test_while_loop_expression_condition(mock_action_executor):
...
async def test_while_loop_false_initially():
...
async def test_foreach_loop_variable_collection(mock_action_executor):
...
async def test_foreach_loop_range_collection(mock_action_executor):
...
async def test_foreach_loop_empty_collection():
...
async def test_foreach_loop_max_iterations_limit(mock_action_executor):
...
async def test_if_then_true_condition(mock_action_executor):
    ...
    result = await executor.execute_if(action)
...
async def test_if_else_false_condition(mock_action_executor):
...
async def test_if_no_else_branch(mock_action_executor):
...
async def test_if_expression_condition(mock_action_executor):
...
async def test_break_unconditional():
    ...
    with pytest.raises(BreakLoop) as exc_info:
        await executor.execute_break(action)

    assert "Breaking now" in str(exc_info.value)


async def test_break_conditional_true():
...
async def test_break_conditional_false():
...
async def test_continue_unconditional():
    ...
    with pytest.raises(ContinueLoop) as exc_info:
        await executor.execute_continue(action)

    assert "Continuing" in str(exc_info.value)


async def test_continue_conditional_true():
...
async def test_continue_conditional_false():
...
def test_condition_operators():
    """Test all comparison operators via the module-level compare_values helper."""
    test_cases = [
        ("==", 10, True),
        ("==", 5, False),
        ("!=", 5, True),
    ...
    for operator, expected, should_match in test_cases:
        result = compare_values(10, operator, expected)
        assert (
            result == should_match
...
def test_condition_contains_operator():
    """Test contains operator via the module-level compare_values helper."""
    assert compare_values("hello world", "contains", "world") is True
    assert compare_values("hello world", "contains", "goodbye") is False


def test_condition_matches_operator():
    """Test regex matches operator via the module-level compare_values helper."""
    assert compare_values("test123", "matches", r"test\d+") is True
    assert compare_values("test", "matches", r"test\d+") is False


async def test_expression_with_variables():
...
async def test_invalid_loop_type():
    ...
    with pytest.raises(ValidationError):
        await executor.execute_loop(action)


async def test_for_loop_missing_iterations():
...
async def test_while_loop_missing_condition():
...
async def test_foreach_loop_nonexistent_variable():
...
async def test_nested_loop_simulation(mock_action_executor):
    ...
    outer_result = await executor.execute_loop(outer_action)

    assert outer_result["success"] is True
    assert outer_result["iterations_completed"] == 3


async def test_loop_with_if_simulation(mock_action_executor):
