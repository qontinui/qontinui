"""Matches collection - Rich container for match results.

Provides sorting, filtering, and querying operations on collections of matches.
"""
...
class Matches:
    """Container for multiple match results with rich operations.

    Provides methods for working with collections of matches including:
    - Sorting: by_similarity, by_position
    - Filtering: by_similarity, by_region, by_distance, by_predicate
    - Queries: first, last, best, worst, nearest_to, farthest_from
    - Operations: remove_overlapping, add, clear
    """
    ...
    # =========================================================================
    # Sorting (in-place, returns self for chaining)
    # =========================================================================
    ...
    def sort_by_position(self, top_to_bottom: bool = True, left_to_right: bool = True) -> "Matches":
        ...
        def position_key(match: Match) -> tuple[int, int]:
            target = match.target
            if target is None:
                return (0, 0)
            y = target.y if top_to_bottom else -target.y
            x = target.x if left_to_right else -target.x
            return (y, x)
