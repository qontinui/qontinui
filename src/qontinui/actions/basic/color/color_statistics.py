...
class ColorStatistics:
    ...
    def similarity_score(self, other: 'ColorStatistics') -> float:
        ...
        h_diff = min(abs(self.h_mean - other.h_mean), 
                    360 - abs(self.h_mean - other.h_mean)) / 180.0
...
class ColorStatisticsAnalyzer:
    ...
    def analyze_image(self, image: np.ndarray) -> ColorStatistics:
        ...
        logger.debug(f"Color statistics: H={stats.h_mean:.1f}±{stats.h_std:.1f}, "
                    f"S={stats.s_mean:.1f}±{stats.s_std:.1f}, "
                    f"V={stats.v_mean:.1f}±{stats.v_std:.1f}")
        
        return stats
    
    def create_color_mask(self, image: np.ndarray, stats: ColorStatistics, 
                         tolerance_std: float = 2.0) -> np.ndarray:
