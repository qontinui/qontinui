...
class HSVBinOptions:
    ...
    def validate(self) -> bool:
        ...
        Returns:
            True if valid
        """
        return (self.hue_bins > 0 and 
                self.saturation_bins > 0 and 
                self.value_bins > 0)
...
class ColorFindOptions(BaseFindOptions):
    ...
    def with_hsv_bins(self, hue: int = 12, saturation: int = 2, value: int = 1) -> 'ColorFindOptions':
    ...
    def kmeans_defaults() -> 'ColorFindOptions':
        ...
        return ColorFindOptions(
            color_strategy=ColorStrategy.KMEANS,
            kmeans=3,
            diameter=10
        )
