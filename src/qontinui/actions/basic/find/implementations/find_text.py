...
from ..options.text_find_options import TextFindOptions, TextMatchType, OCREngine, TextPreprocessing
...
class OCRResult:
    ...
    confidence: float
    region: Region
    word_boxes: Optional[List[Tuple[str, Region]]] = None
...
class FindText:
    ...
    _tesseract: Optional[Any] = field(default=None, init=False)
    _easyocr_reader: Optional[Any] = field(default=None, init=False)
    _paddleocr: Optional[Any] = field(default=None, init=False)
    ...
    def find(self, object_collection: ObjectCollection,
             options: TextFindOptions) -> List[Match]:
        """Find text using OCR.
    ...
    def _get_search_texts(self, object_collection: ObjectCollection,
                         options: TextFindOptions) -> List[str]:
    ...
    def _ocr_tesseract(self, image: np.ndarray, region: Region,
                      options: TextFindOptions) -> List[OCRResult]:
        ...
        try:
            ...
            data = pytesseract.image_to_data(image, lang=options.language,
    ...
    def _ocr_easyocr(self, image: np.ndarray, region: Region,
                    options: TextFindOptions) -> List[OCRResult]:
    ...
    def _ocr_paddleocr(self, image: np.ndarray, region: Region,
                       options: TextFindOptions) -> List[OCRResult]:
    ...
    def _match_text(self, search_text: str, ocr_results: List[OCRResult],
                   options: TextFindOptions) -> List[Match]:
        ...
        for ocr_result in ocr_results:
            similarity = self._calculate_text_similarity(
                search_text, ocr_result.text, options
            )
            
            if similarity >= options.similarity:
                match = Match(
                    location=ocr_result.region.center,
                    region=ocr_result.region,
                    similarity=similarity,
                    text=ocr_result.text if options.return_text_content else None,
                    confidence=ocr_result.confidence if options.return_confidence else None
                )
                matches.append(match)
        
        return matches
    
    def _calculate_text_similarity(self, search_text: str, found_text: str,
                                  options: TextFindOptions) -> float:
    ...
    def _preprocess_image(self, image: np.ndarray,
                         options: TextFindOptions) -> np.ndarray:
    ...
    def _capture_region(self, region: Region) -> Optional[np.ndarray]:
