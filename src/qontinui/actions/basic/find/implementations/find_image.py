...
from ..options.pattern_find_options import PatternFindOptions, MatchMethod
...
class FindImage:
    ...
    def find(self, object_collection: ObjectCollection, 
             options: PatternFindOptions) -> List[Match]:
    ...
    def _get_patterns(self, object_collection: ObjectCollection,
                     options: PatternFindOptions) -> List[Pattern]:
    ...
    def _find_pattern(self, pattern: Pattern, search_images: List[np.ndarray],
                     options: PatternFindOptions) -> List[Match]:
        ...
        if template is None:
            logger.warning(f"Could not load pattern image")
            return []
    ...
    def _single_scale_match(self, template: np.ndarray, image: np.ndarray,
                           options: PatternFindOptions) -> List[Match]:
        ...
        if options.search_type.name == "ALL":
            ...
            for pt in zip(*locations[::-1]):
                match = Match(
                    location=Location(pt[0] + w//2, pt[1] + h//2),
                    region=Region(pt[0], pt[1], w, h),
                    similarity=float(result[pt[1], pt[0]])
                )
            ...
            if confidence >= options.similarity:
                match = Match(
                    location=Location(top_left[0] + w//2, top_left[1] + h//2),
                    region=Region(top_left[0], top_left[1], w, h),
    ...
    def _multiscale_match(self, template: np.ndarray, image: np.ndarray,
                         options: PatternFindOptions) -> List[Match]:
    ...
    def _load_pattern_image(self, pattern: Pattern) -> Optional[np.ndarray]:
        """Load pattern image.
        ...
        logger.debug(f"Loading pattern image")
        return None
