...
class Vanish(ActionInterface):
    ...
    def perform(self, action_result: ActionResult, *object_collections: ObjectCollection) -> None:
        ...
        action_result.set_success(vanished)
        if vanished:
            action_result.text = "Element(s) vanished successfully"
        else:
            action_result.text = f"Element(s) still present after {vanish_options.get_max_wait_time()} seconds"
    
    def _wait_for_vanish(self,
    ...
    def _elements_are_gone(self,
                          action_result: ActionResult,
                          object_collections: tuple) -> bool:
