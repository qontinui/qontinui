...
if TYPE_CHECKING:
    from ..model.state import StateImage, StateLocation, StateRegion, StateString, State
    from ..model.element import Location, Region, Pattern, Scene
...
class ObjectCollection:
    ...
    def is_empty(self) -> bool:
        ...
        """
        return (len(self.state_locations) == 0 and
                len(self.state_images) == 0 and
                len(self.state_regions) == 0 and
                len(self.state_strings) == 0 and
    ...
    def get_first_object_name(self) -> str:
        ...
        if self.state_images:
            ...
            elif self.state_images[0].get_patterns() and self.state_images[0].get_patterns()[0].get_imgpath():
    ...
    def __str__(self) -> str:
        ...
        return (f"ObjectCollection{{stateLocations={len(self.state_locations)}, "
    ...
    def with_strings(self, *strings) -> 'ObjectCollectionBuilder':
        ...
        for string in strings:
            if isinstance(string, str):
                self.state_strings.append(StateString.InNullState().with_string(string))
