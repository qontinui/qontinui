...
class MockFind:
    ...
    def find(self, pattern: Pattern, search_region: Optional[Region] = None) -> ActionResult:
        ...
        logger.debug(f"MockFind: Finding pattern '{pattern.name}' with active states: {active_states}")
    ...
    def _get_snapshot_from_history(self, pattern: Pattern, 
                                  active_states: Set[str]) -> Optional[ActionRecord]:
    ...
    def _generate_mock_result(self, pattern: Pattern, search_region: Optional[Region],
                             active_states: Set[str]) -> ActionResult:
        ...
        if pattern.owner_state_name and pattern.owner_state_name != "NULL":
            probability = self.state_management.get_state_probability(pattern.owner_state_name) / 100.0
    ...
    def _create_mock_match(self, pattern: Pattern, search_region: Optional[Region]) -> Match:
    ...
    def find_all(self, pattern: Pattern, search_region: Optional[Region] = None) -> List[Match]:
    ...
    def wait_for(self, pattern: Pattern, timeout: float = 5.0) -> Optional[Match]:
