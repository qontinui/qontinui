...
class VectorStore:
    ...
    def search(
    ...
    ) -> Tuple[np.ndarray, np.ndarray, List[List[VectorMetadata]]]:
        ...
        try:
            ...
            if threshold is not None:
                ...
                distances = [d[m] for d, m in zip(distances, valid_mask)]
                indices = [i[m] for i, m in zip(indices, valid_mask)]
    ...
    def search_by_id(
        ...
        if not include_self:
            ...
            metadata = [m for i, m in zip(indices[0], metadata[0]) if i != faiss_idx][:k]
        else:
    ...
    def update_vector(
        ...
        new_metadata: Optional[VectorMetadata] = None
    ):
