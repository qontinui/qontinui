...
class QontinuiSettings(BaseSettings):
    ...
    similarity_threshold: float = Field(0.85, ge=0.0, le=1.0, 
                                       description="Default similarity threshold for matching")
    ...
    # Vector DB settings
    vector_db_type: Literal["faiss", "qdrant"] = Field("faiss", 
                                                       description="Vector database backend")
    vector_index_path: Path = Field(Path("./vectors"), 
    ...
    default_monitor: Optional[int] = Field(None, description="Default monitor index")
    
    # RAG settings
    rag_enabled: bool = Field(True, description="Enable RAG-based state recognition")
    semantic_weight: float = Field(0.6, ge=0.0, le=1.0, 
                                  description="Weight for semantic matching")
    deterministic_weight: float = Field(0.4, ge=0.0, le=1.0, 
                                       description="Weight for deterministic matching")
    min_semantic_confidence: float = Field(0.6, ge=0.0, le=1.0,
                                          description="Minimum confidence for semantic matches")
    ...
    state_transition_timeout: float = Field(60.0, 
                                           description="Timeout for state transitions")
    auto_discover_states: bool = Field(False, 
                                      description="Enable automatic state discovery")
...
# Singleton instance
_settings: Optional[QontinuiSettings] = None


def get_settings(env: Optional[str] = None) -> QontinuiSettings:
