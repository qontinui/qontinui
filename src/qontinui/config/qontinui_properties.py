...
class CoreConfig(BaseModel):
    ...
    image_path: str = Field(
        default="classpath:images/",
        description="Path to image resources"
    )
    mock: bool = Field(
        default=False,
        description="Enable mock mode for testing without GUI"
    )
    headless: bool = Field(
        default=False,
        description="Run in headless mode without display"
    )
    sikuli_jar_path: Optional[str] = Field(
        default=None,
        description="Path to SikuliX jar file"
    )
    ...
    image_cache_size: int = Field(
        default=100,
        ge=0,
        description="Maximum number of images to cache"
    )
...
class MouseConfig(BaseModel):
    ...
    move_delay: float = Field(
        default=0.5,
        ge=0,
        description="Delay for mouse movement in seconds"
    )
    pause_before_down: float = Field(
        default=0.0,
        ge=0,
        description="Pause before mouse down in seconds"
    )
    pause_after_down: float = Field(
        default=0.0,
        ge=0,
        description="Pause after mouse down in seconds"
    )
    pause_before_up: float = Field(
        default=0.0,
        ge=0,
        description="Pause before mouse up in seconds"
    )
    pause_after_up: float = Field(
        default=0.0,
        ge=0,
        description="Pause after mouse up in seconds"
    )
    click_delay: float = Field(
        default=0.0,
        ge=0,
        description="Delay between clicks in double-click"
    )
    drag_delay: float = Field(
        default=0.5,
        ge=0,
        description="Delay during drag operations"
    )
...
class MockConfig(BaseModel):
    ...
    click_duration: float = Field(
        default=0.5,
        ge=0,
        description="Simulated click duration in seconds"
    )
    type_duration: float = Field(
        default=2.0,
        ge=0,
        description="Simulated typing duration in seconds"
    )
    find_duration: float = Field(
        default=0.3,
        ge=0,
        description="Simulated find duration in seconds"
    )
    drag_duration: float = Field(
        default=1.0,
        ge=0,
        description="Simulated drag duration in seconds"
    )
    scroll_duration: float = Field(
        default=0.5,
        ge=0,
        description="Simulated scroll duration in seconds"
    )
    wait_duration: float = Field(
        default=0.1,
        ge=0,
        description="Simulated wait duration in seconds"
    )
    vanish_duration: float = Field(
        default=1.0,
        ge=0,
        description="Simulated vanish check duration in seconds"
    )
    exists_duration: float = Field(
        default=0.3,
        ge=0,
        description="Simulated exists check duration in seconds"
    )
...
class ScreenshotConfig(BaseModel):
    ...
    max_history: int = Field(
        default=50,
        ge=0,
        description="Maximum screenshot history to maintain"
    )
    format: str = Field(
        default="png",
    ...
    capture_on_error: bool = Field(
        default=True,
        description="Automatically capture screenshot on error"
    )
...
class IllustrationConfig(BaseModel):
    ...
    enabled: bool = Field(
        default=True,
        description="Enable action illustrations"
    )
    show_click: bool = Field(
        default=True,
        description="Illustrate click actions"
    )
    show_drag: bool = Field(
        default=True,
        description="Illustrate drag actions"
    )
    show_type: bool = Field(
        default=True,
        description="Illustrate type actions"
    )
    show_find: bool = Field(
        default=True,
        description="Illustrate find operations"
    )
    highlight_color: str = Field(
        default="red",
        description="Color for highlighting elements"
    )
    highlight_thickness: int = Field(
        default=3,
        ge=1,
        le=10,
        description="Highlight border thickness"
    )
...
class AnalysisConfig(BaseModel):
    ...
    hsv_bins: List[int] = Field(
        default=[50, 60, 60],
        description="HSV histogram bins [hue, saturation, value]"
    )
    min_contour_area: int = Field(
        default=100,
        ge=1,
        description="Minimum contour area for color regions"
    )
    max_contour_area: int = Field(
        default=100000,
        ge=1,
        description="Maximum contour area for color regions"
    )
...
class RecordingConfig(BaseModel):
    ...
    path: str = Field(
        default="recordings/",
        description="Path to save recordings"
    )
    fps: int = Field(
        default=30,
        ge=1,
...
class DatasetConfig(BaseModel):
    ...
    collect: bool = Field(
        default=False,
        description="Enable dataset collection"
    )
    path: str = Field(
        default="datasets/",
        description="Path to save datasets"
    )
    include_screenshots: bool = Field(
        default=True,
        description="Include screenshots in dataset"
    )
    include_actions: bool = Field(
        default=True,
        description="Include action data in dataset"
    )
    ...
    include_results: bool = Field(
        default=True,
        description="Include action results"
    )
    format: str = Field(
        default="json",
...
class TestingConfig(BaseModel):
    ...
    timeout_multiplier: float = Field(
        default=2.0,
        ge=1.0,
        description="Multiply timeouts during testing"
    )
    ...
    max_retries: int = Field(
        default=3,
        ge=0,
        description="Maximum test retry attempts"
    )
...
class QontinuiProperties(BaseModel):
    ...
    mouse: MouseConfig = Field(
        default_factory=MouseConfig,
        description="Mouse action configuration"
    )
    mock: MockConfig = Field(
        default_factory=MockConfig,
        description="Mock mode timing configuration"
    )
    screenshot: ScreenshotConfig = Field(
        default_factory=ScreenshotConfig,
        description="Screenshot and history settings"
    )
    illustration: IllustrationConfig = Field(
        default_factory=IllustrationConfig,
        description="Action illustration settings"
    )
    analysis: AnalysisConfig = Field(
        default_factory=AnalysisConfig,
        description="Color analysis settings"
    )
    ...
    dataset: DatasetConfig = Field(
        ...
        description="AI dataset generation settings"
    )
    ...
    def to_env_file(self, path: Optional[Path] = None) -> str:
