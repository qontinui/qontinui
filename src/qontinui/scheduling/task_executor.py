...
class TaskExecutor:
    ...
    def execute(self, task: ScheduledTask, timeout: Optional[float] = None) -> bool:
    ...
    def execute_with_retry(self, task: ScheduledTask, 
        ...
        for attempt in range(max_retries + 1):
            if attempt > 0:
                logger.info(f"Retrying task '{task.name}' (attempt {attempt + 1}/{max_retries + 1})")
