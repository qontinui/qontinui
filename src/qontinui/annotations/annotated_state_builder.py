...
class AnnotatedStateBuilder:
    ...
    def build_state(self, state_instance: Any, state_metadata: Dict[str, Any]) -> State:
        ...
        if components.state_images:
            ...
            logger.trace(f"Added {len(components.state_images)} StateImages to state '{state_name}'")
        
        if components.state_strings:
            state_builder.with_strings(*components.state_strings)
            logger.trace(f"Added {len(components.state_strings)} StateStrings to state '{state_name}'")
        ...
        logger.info(f"Built state '{state_name}' with {components.get_total_components()} total components")
        
        return state
    
    def _set_owner_state_name_for_components(self,
                                            components: 'StateComponentExtractor.StateComponents',
                                            state_name: str) -> None:
        ...
        for state_string in components.state_strings:
            ...
            logger.trace(f"Set owner state '{state_name}' for StateString '{state_string.name}'")
        ...
        for state_object in components.state_objects:
            ...
            if isinstance(state_object, StateLocation):
                ...
                logger.trace(f"Set owner state '{state_name}' for StateLocation '{state_object.name}'")
            elif isinstance(state_object, StateRegion):
                state_object.owner_state_name = state_name
                logger.trace(f"Set owner state '{state_name}' for StateRegion '{state_object.name}'")
            else:
                logger.trace(
        ...
        logger.debug(f"Set owner state name '{state_name}' for {components.get_total_components()} components")
