...
class DriftDetector:
    ...
    def template_anchor_similarity(
        ...
        try:
            ...
            return max(0.0, min(1.0, (score + 1.0) / 2.0)) if score < 0 else max(
                0.0, min(1.0, score)
            )
        except ImportError:
            pass
    ...
    def check(
        ...
        iou_confidence = min(1.0, iou_val / max(iou_threshold, 1e-6) * 0.5) if (
            iou_val < iou_threshold
        ) else min(1.0, 0.5 + (iou_val - iou_threshold) * 0.5 / max(
            1.0 - iou_threshold, 1e-6
        ))
