...
class StateStore:
    ...
    def register(self, state: State, parent: Optional[str] = None) -> bool:
    ...
    def get(self, identifier: Union[str, StateEnum]) -> Optional[State]:
    ...
    def get_active_states(self) -> List[State]:
        ...
        """
        with self._lock:
            return [self._states[name] for name in self._active_states 
                   if name in self._states]
    ...
    def add_transition(self, from_state: str, to_state: str, 
        ...
        with self._lock:
            if from_state not in self._states or to_state not in self._states:
                logger.error(f"Cannot add transition: states not found")
                return False
    ...
    def get_metadata(self, name: str) -> Optional[StateMetadata]:
    ...
    def get_parent(self, child: str) -> Optional[str]:
        ...
        with self._lock:
            return {
                ...
                "success_rate": (self._total_transitions - self._failed_transitions) / 
                               max(1, self._total_transitions) * 100
            }
    ...
    def __str__(self) -> str:
        ...
        with self._lock:
            return (f"StateStore({len(self._states)} states, "
    ...
    def __repr__(self) -> str:
        ...
        with self._lock:
            return (f"StateStore(states={len(self._states)}, "
