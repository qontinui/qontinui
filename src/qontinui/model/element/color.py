...
class HSV:
    ...
    def is_similar(self, other: 'HSV', hue_tolerance: int = 10, 
                   sat_tolerance: int = 20, val_tolerance: int = 20) -> bool:
...
class ColorRange:
    ...
    min_color: Optional[RGB] = None
    """Minimum RGB color."""
    
    max_color: Optional[RGB] = None  
    """Maximum RGB color."""
    
    min_hsv: Optional[HSV] = None
    """Minimum HSV color."""
    
    max_hsv: Optional[HSV] = None
    """Maximum HSV color."""
    ...
    def contains_rgb(self, color: RGB) -> bool:
        ...
        return (self.min_color.red <= color.red <= self.max_color.red and
                self.min_color.green <= color.green <= self.max_color.green and
                self.min_color.blue <= color.blue <= self.max_color.blue)
    ...
    def contains_hsv(self, color: HSV) -> bool:
        ...
        return (in_hue_range and
                self.min_hsv.saturation <= color.saturation <= self.max_hsv.saturation and
                self.min_hsv.value <= color.value <= self.max_hsv.value)
