...
class ActionExecutor:
    ...
    def _find_image_on_screen(
        self, image_path: str, threshold: float = 0.9
    ) -> tuple[int, int] | None:
        """Find image on screen using template matching."""
        try:
            # Take screenshot using HAL
    ...
    def _execute_click(self, action: Action) -> bool:
        ...
        if location:
            ...
            self.input_controller.click_at(location[0], location[1], MouseButton.LEFT)
    ...
    def _execute_double_click(self, action: Action) -> bool:
        ...
        if location:
            ...
            self.input_controller.double_click_at(location[0], location[1], MouseButton.LEFT)
    ...
    def _execute_right_click(self, action: Action) -> bool:
        ...
        if location:
            ...
            self.input_controller.click_at(location[0], location[1], MouseButton.RIGHT)
    ...
    def _execute_drag(self, action: Action) -> bool:
        ...
        if start and destination:
            ...
            self.input_controller.drag(start[0], start[1], end_x, end_y, duration)
    ...
    def _execute_scroll(self, action: Action) -> bool:
        ...
        location = self._get_target_location(action.config)
        if location:
            self.input_controller.move_mouse(location[0], location[1])
