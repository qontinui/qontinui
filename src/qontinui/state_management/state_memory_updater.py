...
class StateMemoryUpdater:
    ...
    def update_from_match(self, match: Match) -> Optional[str]:
    ...
    def _activate_from_match(self, match: Match) -> Optional[str]:
    ...
    def _get_state_from_match(self, match: Match) -> Optional[str]:
    ...
    def deactivate_states_without_matches(
        ...
        states_to_check: Optional[Set[str]] = None
    ) -> Set[str]:
        ...
        for state_name in states_to_check:
            if state_name not in states_with_matches and self.state_memory.is_state_active(state_name):
...
_state_memory_updater: Optional[StateMemoryUpdater] = None
