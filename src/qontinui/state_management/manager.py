...
from .models import State, Element, Transition, TransitionType, StateGraph
import numpy as np
...
class QontinuiStateManager:
    ...
    def add_state(self, state: State, parent: Optional[str] = None):
        ...
        if self.state_evidence[state_name] > self.activation_threshold:
            if state_name not in self.active_states:
                ...
                try:
                    ...
                    logger.debug(f"State {state_name} activated without machine transition")
    ...
    def update_evidence(self, current_elements: List[Element]):
        ...
        for state_name, state in self.state_graph.states.items():
            ...
            elif state_name in self.active_states and \
                 self.state_evidence.get(state_name, 0) < self.deactivation_threshold:
                self.deactivate_state(state_name)
    
    def _calculate_state_evidence(self, state: State, 
                                 current_elements: List[Element]) -> float:
    ...
    def _calculate_element_similarity(self, elem1: Element, 
                                    elem2: Element) -> float:
    ...
    def _create_transition_conditions(self, transition: Transition) -> List[Callable]:
        ...
        for condition_str in transition.conditions:
    ...
    def _execute_callbacks(self, callback_dict: Dict[str, List[Callable]], 
                         state_name: str):
