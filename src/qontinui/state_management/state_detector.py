...
class StateDetector:
    ...
    def check_for_active_states(self) -> None:
        ...
        if states_no_longer_visible and self.all_states_in_project_service:
            ...
            logger.info(f"Removed {len(states_no_longer_visible)} states that are no longer visible: "
                       f"{', '.join(state_names)}")
    ...
    def rebuild_active_states(self) -> None:
        ...
        if self.state_memory and self.state_memory.active_states:
            logger.info(f"Active states still present after verification: "
        ...
        if self.state_memory:
            if not self.state_memory.active_states:
                logger.warning("No states found after comprehensive search, defaulting to UNKNOWN state")
    ...
    def find_state_by_name(self, state_name: str) -> bool:
        ...
        result = self.action.find(
            ObjectCollection.Builder()
                .with_non_shared_images(state)
                .build()
        )
