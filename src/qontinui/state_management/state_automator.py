...
from ..annotations.state import is_state, get_state_metadata
from ..annotations.transition_method import (
    collect_transitions,
    create_code_transition
)
...
class StateAutomator:
    ...
    def __init__(self, actions: Optional[Actions] = None, state_store: Optional[StateStore] = None):
        ...
        for state_obj in state_instances:
            try:
                ...
                if hasattr(state_obj, 'state') and isinstance(state_obj.state, State):
                    ...
                    logger.warning(f"State instance {state_obj} does not have a valid 'state' attribute")
            ...
            try:
                ...
                for trans_meta in transitions:
                    ...
                    if from_state:
                        ...
                        logger.debug(f"Found transition: {trans_meta['name']} "
    ...
    def execute_transition(self, transition_name: str) -> bool:
        ...
        for trans in transitions:
            if trans.name == transition_name:
                logger.info(f"Executing transition: {transition_name} "
    ...
    def get_transition_history(self, state_name: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def __str__(self) -> str:
        ...
        return (f"StateAutomator({len(self._registered_states)} states, "
                f"{sum(len(t) for t in self._registered_transitions.values())} transitions)")
    
    def __repr__(self) -> str:
        """Developer representation."""
        return (f"StateAutomator(states={self._registered_states}, "
