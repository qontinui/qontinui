...
class RetryPolicy:
    ...
    non_recoverable_exceptions: Optional[List[Type[Exception]]] = None
...
class ErrorRecoveryAspect:
                ...
                for attempt in range(retry_policy.max_attempts):
                    try:
                        ...
                        if attempt > 0:
                            ...
                            logger.info(f"Recovery successful for {method_name} on attempt {attempt + 1}")
    ...
    def enable_circuit_breaker(self,
                              ...
                              reset_timeout_seconds: int = 60) -> None:
...
def with_error_recovery(policy: Optional[RetryPolicy] = None,
                       fallback: Optional[Callable] = None) -> Callable:
