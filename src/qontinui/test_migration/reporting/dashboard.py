...
try:
    from ..core.models import TestResults, TestResult
    from ..validation.coverage_tracker import CoverageTracker
    from ..validation.diagnostic_reporter import DiagnosticReporter
except ImportError:
...
class MigrationReportingDashboard:
    ...
    def _generate_summary(self, test_directory: Path) -> Dict[str, Any]:
        ...
        # Count test files
        test_files = list(test_directory.rglob("test_*.py")) + list(test_directory.rglob("*_test.py"))
    ...
    def _save_html_report(self, report_data: Dict[str, Any], output_file: Path, template_file: Optional[Path]):
        """Save HTML report."""
    ...
    def _save_pdf_report(self, report_data: Dict[str, Any], output_file: Path):
        ...
        try:
            ...
            raise ImportError("ReportLab is required for PDF output")
    ...
    def _format_text_report(self, report_data: Dict[str, Any]) -> str:
        ...
        if complexity:
            ...
            lines.append(f"    Average File Size: {complexity.get('average_file_size', 0):.1f} lines")
