...
class TestBrobotMockAnalyzer:
    ...
    def test_identify_annotation_mocks(self):
        ...
        with patch.object(Path, 'exists', return_value=False):
            with patch.object(self.analyzer, '_reconstruct_file_content', return_value=java_content):
                mock_usages = self.analyzer.identify_mock_usage(test_file)
        
        # Should find Brobot-related mocks
        brobot_mocks = [m for m in mock_usages if 'Brobot' in m.mock_class or 'State' in m.mock_class or 'Action' in m.mock_class]
    ...
    def test_get_mock_dependency_mapping(self):
        ...
        assert mappings['io.github.jspinak.brobot.mock'] == 'qontinui.test_migration.mocks.brobot_mocks'
    ...
    def test_analyze_mock_complexity(self):
        ...
        moderate_mock = MockUsage(
            mock_type="brobot_programmatic_mock",
            mock_class="StateObject",
            configuration={'variable_name': 'state', 'creation_line': 'mock(StateObject.class)'}
        )
