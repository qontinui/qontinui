...
class TestAssertionConverter:
    ...
    def test_assert_not_equals_conversion(self):
        ...
        result = self.converter.convert_assertion('assertNotEquals(null, result, "Should not be null")')
    ...
    def test_assert_same_conversion(self):
        ...
        result = self.converter.convert_assertion('assertSame(instance1, instance2, "Should be same object")')
    ...
    def test_assert_array_equals_conversion(self):
        ...
        result = self.converter.convert_assertion('assertArrayEquals(expected, actual, "Arrays should match")')
    ...
    def test_assert_throws_conversion(self):
        ...
        result = self.converter.convert_assertion("assertThrows(IllegalArgumentException.class, () -> method.call())")
        expected = "with pytest.raises(IllegalArgumentException):\n    method.call()"
        assert result == expected
        
        # Test with method reference
        result = self.converter.convert_assertion("assertThrows(RuntimeException.class, service::throwException)")
    ...
    def test_parameter_extraction(self):
        ...
        # Simple parameters
        params, message = self.converter._extract_assertion_params("assertTrue(condition)", "assertTrue")
        assert params == ["condition"]
        assert message is None
        
        # Parameters with message
        params, message = self.converter._extract_assertion_params('assertEquals(5, result, "Error")', "assertEquals")
        assert params == ["5", "result"]
        assert message == '"Error"'
        
        # Complex parameters with nested calls
        params, message = self.converter._extract_assertion_params("assertEquals(service.getValue(), result.getActual())", "assertEquals")
    ...
    def test_string_literal_detection(self):
        ...
        assert self.converter._is_string_literal('"Hello"') == True
        assert self.converter._is_string_literal("'World'") == True
        assert self.converter._is_string_literal("variable") == False
        assert self.converter._is_string_literal("123") == False
    ...
    def test_custom_assertion_conversion(self):
        ...
        result = self.converter.convert_custom_assertion("assertUserIsValid(user)", "assertUserIsValid")
        assert result == "self.assert_user_is_valid(user)"
        
        result = self.converter.convert_custom_assertion("assertCustomCondition(val1, val2)", "assertCustomCondition")
    ...
    def test_complex_assertion_scenarios(self):
        ...
        result = self.converter.convert_assertion("assertTrue(condition1 && condition2 || condition3)")
        assert result == "assert condition1 && condition2 || condition3"
        
        # Nested method calls in assertEquals
        result = self.converter.convert_assertion("assertEquals(service.getExpectedValue(), processor.calculateResult())")
