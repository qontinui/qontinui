...
from ..core.models import TestFile, TestMethod, Dependency, MockUsage
...
class SpringTestAdapter:
    ...
    def create_application_context_setup(self, test_file: TestFile) -> List[str]:
        ...
        has_spring_boot_test = any('@SpringBootTest' in str(method.annotations) 
    ...
    def _process_annotation(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
    ...
    def _handle_spring_boot_test(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
    ...
    def _handle_test_configuration(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @TestConfiguration annotation."""
        config['imports'].add('import pytest')
        config['setup_code'].append("# Test configuration setup")
    
    def _handle_mock_bean(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @MockBean annotation."""
        config['imports'].add('from unittest.mock import Mock, patch')
        
        bean_class = self._extract_annotation_parameter(annotation, 'value') or \
                    self._extract_annotation_parameter(annotation, 'classes')
        
        if bean_class:
            mock_setup = f"# Mock bean for {bean_class}"
            config['setup_code'].append(mock_setup)
    
    def _handle_spy_bean(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @SpyBean annotation."""
        config['imports'].add('from unittest.mock import Mock, patch')
        config['setup_code'].append("# Spy bean setup")
    
    def _handle_autowired(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @Autowired annotation."""
        config['imports'].add('import pytest')
        # Autowired fields will be handled in dependency injection setup
    
    def _handle_value_injection(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
    ...
    def _handle_test_property_source(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @TestPropertySource annotation."""
        config['imports'].add('import os')
        
        locations = self._extract_annotation_parameter(annotation, 'locations')
        properties = self._extract_annotation_parameter(annotation, 'properties')
        
        if properties:
            config['environment_setup'].extend(self._convert_properties_to_env_vars(properties))
    
    def _handle_active_profiles(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @ActiveProfiles annotation."""
        config['imports'].add('import os')
        
        profiles = self._extract_annotation_parameter(annotation, 'profiles') or \
                  self._extract_annotation_parameter(annotation, 'value')
        
        if profiles:
            config['environment_setup'].append(f"os.environ['ACTIVE_PROFILES'] = '{profiles}'")
    
    def _handle_dirties_context(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @DirtiesContext annotation."""
        config['imports'].add('import pytest')
        
        class_mode = self._extract_annotation_parameter(annotation, 'classMode')
        method_mode = self._extract_annotation_parameter(annotation, 'methodMode')
    ...
    def _handle_transactional(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @Transactional annotation."""
        config['imports'].add('import pytest')
        
        if scope == 'method' and method:
            # Add transaction fixture to method
            config['fixtures'].append(f"@pytest.fixture")
    ...
    def _handle_rollback(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @Rollback annotation."""
        config['imports'].add('import pytest')
        config['setup_code'].append("# Rollback configuration")
    
    def _handle_sql_annotation(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @Sql annotation."""
        config['imports'].add('import pytest')
        
        scripts = self._extract_annotation_parameter(annotation, 'scripts') or \
                 self._extract_annotation_parameter(annotation, 'value')
        
        if scripts:
            config['setup_code'].append(f"# Execute SQL scripts: {scripts}")
    
    def _handle_data_jpa_test(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
    ...
    def _handle_web_mvc_test(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @WebMvcTest annotation."""
        config['imports'].add('import pytest')
        config['imports'].add('from unittest.mock import Mock')
        config['setup_code'].append("# Web MVC test configuration")
    
    def _handle_json_test(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
        """Handle @JsonTest annotation."""
        config['imports'].add('import pytest')
        config['imports'].add('import json')
        config['setup_code'].append("# JSON test configuration")
    
    def _handle_test_method_order(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
    ...
    def _handle_test_instance(self, annotation: str, config: Dict, scope: str, method: Optional[TestMethod] = None):
    ...
    def _extract_annotation_parameter(self, annotation: str, parameter_name: str) -> Optional[str]:
    ...
    def _extract_property_name(self, value_expression: str) -> Optional[str]:
...
class DependencyContainer:
    ...
    def register_component(self, component_type: str, component_instance: any, singleton: bool = True):
        """
