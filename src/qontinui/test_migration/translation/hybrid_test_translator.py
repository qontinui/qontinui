...
try:
    ...
    from .llm_test_translator import LLMTestTranslator, LLMTranslationResult
except ImportError:
...
class HybridTestTranslator(TestTranslator):
    ...
    def translate_test_method(self, method_code: str) -> str:
        ...
        try:
            ...
            if self.llm_translator:
                ...
                raise TranslationException("Both utility and LLM translation failed")
    ...
    def translate_assertions(self, assertion_code: str) -> str:
        ...
        try:
            ...
            if self.llm_translator:
                ...
                raise TranslationException("Both utility and LLM assertion translation failed")
    
    def _determine_optimal_strategy(
        self, 
        test_file: TestFile, 
        requested_strategy: TranslationStrategy
    ) -> TranslationStrategy:
        """
        ...
        if requested_strategy in [TranslationStrategy.HYBRID_UTILITY_FIRST, TranslationStrategy.HYBRID_LLM_FIRST]:
    ...
    def _calculate_complexity_score(self, test_file: TestFile) -> float:
        ...
        for method in test_file.test_methods:
            ...
            # Complex assertions
            if any(keyword in method.body for keyword in ['when(', 'verify(', 'thenReturn', 'doThrow']):
                score += 0.2
    ...
    def _translate_utility_only(self, test_file: TestFile) -> TranslationResult:
        ...
        try:
            ...
            raise TranslationException(f"Utility translation failed: {str(e)}")
    ...
    def _translate_llm_only(self, test_file: TestFile) -> TranslationResult:
        ...
        try:
            ...
            raise TranslationException(f"LLM translation failed: {str(e)}")
