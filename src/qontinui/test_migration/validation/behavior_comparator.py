...
class BehaviorComparatorImpl(BehaviorComparator):
    ...
    def __init__(self, isolation_config: Optional[TestIsolationConfig] = None):
        """
    ...
    def _perform_detailed_comparison(
        ...
        if java_result.passed and python_result.passed:
            ...
            if not output_match:
                ...
                differences.extend(self._generate_output_diff(
                    java_result.output, 
        ...
        # Compare error messages if both tests failed
            ...
            if java_result.error_message and python_result.error_message:
                error_similarity = self._compare_error_messages(
                    java_result.error_message,
                    python_result.error_message
                )
    ...
    def _initialize_output_normalizers(self) -> Dict[str, List[callable]]:
        ...
        return {
            "java": [
                ...
                lambda x: re.sub(r'Exception in thread "[^"]*"', 'Exception', x),  # Normalize exceptions
            ],
            "python": [
                lambda x: re.sub(r'Traceback \(most recent call last\):', 'Exception:', x),  # Normalize tracebacks
                lambda x: re.sub(r'  File "[^"]+", line \d+, in [^\n]+\n', '', x),  # Remove traceback lines
            ]
        }
