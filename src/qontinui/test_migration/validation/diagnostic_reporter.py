...
class DiagnosticReporterImpl(DiagnosticReporter):
    ...
    def generate_migration_summary(self, results: TestResults) -> str:
        ...
        summary_lines.append(f"Success Rate: {(results.passed_tests / results.total_tests * 100):.1f}%")
    ...
    def detect_dependency_differences(self, java_test: TestFile, python_test_path: Path) -> List[DependencyDifference]:
        """
        ...
        for java_dep in java_test.dependencies:
            ...
            if python_equivalent:
                ...
                if python_equivalent not in python_imports:
                    differences.append(DependencyDifference(
    ...
    def detect_setup_differences(self, java_test: TestFile, python_test_path: Path) -> List[SetupDifference]:
        """
            ...
            for annotation in method.annotations:
                ...
                if python_equivalent:
                    ...
                    if python_equivalent not in python_content:
                        differences.append(SetupDifference(
    ...
    def compare_assertion_logic(self, java_test: TestFile, python_test_path: Path) -> List[AssertionDifference]:
        """
        ...
        for method in java_test.test_methods:
            for java_assertion in method.assertions:
                ...
                if python_equivalent:
                    ...
                    differences.append(AssertionDifference(
    ...
    def generate_comprehensive_report(self, java_test: TestFile, python_test_path: Path, 
                                    failure_analysis: Optional[FailureAnalysis] = None) -> DiagnosticReport:
        """
    ...
    def _find_equivalent_python_assertion(self, java_assertion: str, python_assertions: List[str]) -> Optional[str]:
    ...
    def _suggest_assertion_improvement(self, java_assertion: str, python_assertion: str) -> str:
        ...
        if confidence < 0.5:
            return f"Review assertion migration - low confidence match"
        elif confidence < 0.8:
            return f"Consider improving assertion clarity"
        else:
    ...
    def _calculate_migration_completeness(self, dependency_diffs: List[DependencyDifference],
                                        setup_diffs: List[SetupDifference],
                                        assertion_diffs: List[AssertionDifference]) -> float:
    ...
    def _calculate_overall_confidence(self, dependency_diffs: List[DependencyDifference],
                                    setup_diffs: List[SetupDifference],
                                    assertion_diffs: List[AssertionDifference],
                                    failure_analysis: Optional[FailureAnalysis]) -> float:
        """Calculate overall confidence in migration quality."""
        confidence_scores = []
        
        # Dependency confidence
        if dependency_diffs:
            dep_confidence = sum(1.0 if not d.requires_manual_mapping else 0.5 
                               for d in dependency_diffs) / len(dependency_diffs)
            confidence_scores.append(dep_confidence)
        
        # Setup confidence
        if setup_diffs:
            setup_confidence = sum(1.0 if s.migration_status != "missing" else 0.3 
                                 for s in setup_diffs) / len(setup_diffs)
    ...
    def _generate_recommendations(self, dependency_diffs: List[DependencyDifference],
                                setup_diffs: List[SetupDifference],
                                assertion_diffs: List[AssertionDifference],
                                failure_analysis: Optional[FailureAnalysis]) -> List[str]:
...
class DiagnosticReporterImpl(DiagnosticReporter):
    """
    Concrete implementation of DiagnosticReporter interface.
    
    Provides detailed failure analysis reports and migration summaries
    as specified in requirements 6.5, 6.6.
    """
    
    def __init__(self):
        """Initialize the diagnostic reporter."""
        self._java_to_python_mappings = {
            "org.junit.jupiter.api.Test": "pytest",
            "org.junit.jupiter.api.BeforeEach": "pytest.fixture",
            "org.junit.jupiter.api.AfterEach": "pytest.fixture",
            "org.junit.jupiter.api.BeforeAll": "pytest.fixture(scope='session')",
    ...
    def generate_failure_report(self, analysis: FailureAnalysis) -> str:
        ...
        report = []
        report.append("=" * 60)
        report.append("DIAGNOSTIC FAILURE REPORT")
        report.append("=" * 60)
        report.append("")
        
        # Basic failure information
        report.append(f"Migration Issue: {'Yes' if analysis.is_migration_issue else 'No'}")
        report.append(f"Code Issue: {'Yes' if analysis.is_code_issue else 'No'}")
        ...
        if analysis.diagnostic_info:
            ...
            for key, value in analysis.diagnostic_info.items():
    ...
    def generate_migration_summary(self, results: TestResults) -> str:
        ...
        Returns:
            Formatted migration summary as string
        """
        report = []
        report.append("=" * 60)
        ...
        passed_tests = len([r for r in results.test_results if r.passed])
        ...
        if results.execution_time:
            report.append(f"Total Execution Time: {results.execution_time:.2f}s")
            report.append("")
        ...
        # Summary
        if failed_tests == 0:
            report.append("🎉 All tests passed! Migration appears successful.")
        else:
            report.append(f"⚠️  {failed_tests} tests failed. Review required.")
