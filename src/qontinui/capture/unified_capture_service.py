...
from ..hal.config import HALConfig, CaptureBackend
...
class UnifiedCaptureService:
    ...
    def capture_screen(self, screen_id: Optional[int] = None) -> Image.Image:
        ...
        return self._execute_with_retry(
            lambda: self._provider.capture_screen(screen_id),
            "capture_screen"
        )
    ...
    def _execute_with_retry(self, operation: Callable, operation_name: str) -> Any:
        ...
        if not self.config.auto_retry:
            ...
            try:
                ...
                raise ScreenCaptureException(f"{operation_name} failed: {e}")
        ...
        for attempt in range(self.config.retry_count):
            try:
                ...
                elif self.config.fallback_enabled and self._fallback_index < len(self.config.fallback_providers):
...
_unified_capture: Optional[UnifiedCaptureService] = None
