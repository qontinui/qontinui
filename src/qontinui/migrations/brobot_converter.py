...
class BrobotConverter:
    ...
    def convert_all(self) -> ConversionReport:
        ...
        logger.info(f"Conversion complete: {self.report.converted_states}/{self.report.total_images} states converted")
        
        return self.report
    
    def convert_image(self, image_path: Path) -> Optional[State]:
    ...
    def _segment_to_element(self, segment: Dict[str, Any], 
                          state_name: str, index: int) -> Element:
    ...
    def _find_brobot_configs(self) -> List[Path]:
        ...
        configs = [c for c in configs if "config" in c.name.lower() or 
                  "state" in c.name.lower() or "transition" in c.name.lower()]
    ...
    def _extract_transitions(self, config_data: Dict[str, Any], 
                           states: List[State]) -> List[Transition]:
        ...
        if "transitions" in config_data:
            for trans_data in config_data["transitions"]:
                if trans_data.get("from") in state_names and \
                   trans_data.get("to") in state_names:
