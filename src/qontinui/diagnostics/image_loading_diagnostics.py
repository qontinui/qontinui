...
class ImageLoadingDiagnostics:
    ...
    def _generate_performance_report(self) -> Dict[str, Any]:
        ...
        load_times = [r.load_time_ms for r in self.load_history.values() 
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if not libs.get('OpenCV'):
            recommendations.append("Consider installing OpenCV for advanced image operations: pip install opencv-python")
        ...
        if perf.get('failed_loads', 0) > 0:
            recommendations.append(f"{perf['failed_loads']} image load failures detected - check file paths and formats")
        
        avg_load_time = perf.get('average_load_time_ms', 0)
        if avg_load_time > 100:
            recommendations.append(f"High average load time ({avg_load_time:.1f}ms) - consider image optimization")
    ...
    def _print_path_configuration(self, paths: Dict[str, Any]) -> None:
        ...
        logger.info(f"Screenshot Path: {paths['screenshot_path']} (exists: {paths['screenshot_path_exists']})")
        ...
        for result in results:
            if result.success:
                ...
                if result.image_dimensions:
                    logger.info(f"  Dimensions: {result.image_dimensions[0]}x{result.image_dimensions[1]}")
            else:
...
def run_diagnostics(config: Optional[DiagnosticsConfig] = None) -> Dict[str, Any]:
